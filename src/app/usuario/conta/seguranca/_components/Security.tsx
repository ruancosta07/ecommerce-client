    ...
    <section>
      <div>
        <div className="flex gap-[1rem]">
          <SwitchPrimitive.Root aria-checked={checked} checked={checked} disabled={!!user?.provider} onCheckedChange={(e) => {
            if (!user?.provider) {
              switchTwoStepsAuth(e)
            }
          }} className={'bg-zinc-800 w-[5rem] h-[2.5rem] relative p-[.2rem] rounded-full aria-checked:bg-zinc-100 duration-200 disabled:opacity-70 disabled:cursor-not-allowed'}>
